// app/battle/BattleClient.tsx
'use client';

import React, { useEffect, useRef, useState } from 'react';

// Importar Phaser SOLO en el navegador
...
type MapDef = {
  ...
  width: number;
  height: number;
  terrain: string;          // nombre de frame en el atlas terrain64
  buildMask: MapRect[];
  paths: MapPoint[][];
  waves: {
...
async function loadMapDef(name: string): Promise<MapDef> {
  try {
    ...
    console.warn('[TD] No se pudo cargar el mapa', name, err);
    // Fallback simple para que el juego SIEMPRE arranque
    const W = 64;
    const w = 20, h = 10;
    const pathA = Array.from({ length: w }, (_, x) => ({ x, y: 3 }));
    const pathB = Array.from({ length: w }, (_, x) => ({ x, y: 5 }));
    return {
      name: 'fallback',
      tileSize: W,
      width: w,
      ...
      paths: [pathA, pathB],
      waves: {
        baseCount: 8, countPerWave: 2,
        baseHP: 28,  hpPerWave: 8,
        baseSpeed: 52, speedPerWave: 2,
  ...
  return class TD extends Phaser.Scene {
    ...
    async create() {
      try {
        ...
        // Logs útiles para depurar assets/frames
        console.log('[TD] Map loaded:', this.map);
        try {
          const names = (this.textures.get('terrain64') as any)?.getFrameNames?.() ?? [];
          console.log('[TD] terrain64 frames:', names);
        } catch {}
        ...
        this.infoText = this.add.text(16, 34, `Click para colocar – 1=⚡ Electric / 2=🔥 Fire / 3=❄ Frost  · ←/→ cambia skin · pasa el mouse sobre una torre para ver DPS/Range`,
          { color: '#b7c7ff', fontFamily: 'monospace', fontSize: '12px' }).setDepth(1000);
        ...
        console.error('[TD] Error en create()', e);
    ...
    drawMapFromJSON(map: MapDef) {
      ...
      // Bloqueos adicionales (si los hay)
    ...
    setupWavesFromJSON(map: MapDef) {
      ...
      const next = () => {
        ...
        // mini-oleada por el otro carril cada 3
