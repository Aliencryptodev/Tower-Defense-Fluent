...
function createSceneClass() {
  ...
  return class TD extends Phaser.Scene {
    ...
    preload() {
      ...
      // Audio (WAV para evitar 404 si no hay MP3)
      try { this.load.audio('coin',  '/audio/coin.wav'); } catch {}
      try { this.load.audio('hit',   '/audio/hit.wav'); } catch {}
      try { this.load.audio('place', '/audio/place.wav'); } catch {}
      try { this.load.audio('shoot', '/audio/shot.wav'); } catch {}
      try { this.load.audio('music', '/audio/music.wav'); } catch {}
    }

    /* ---------- HELPERS de partículas (Phaser 3.60+) ---------- */

    /** Burst puntual (explosión) */
    burst(x: number, y: number, key: string, frame?: string, cfg: any = {}, qty = 16) {
      const emitter = this.add.particles(x, y, key, frame as any, {
        lifespan: 350,
      ...
      emitter.explode(qty, x, y);
      this.time.delayedCall(500, () => {
        try { emitter.stop(); } catch {}
        try { (emitter as any).manager?.destroy(); } catch {}
      });
      return emitter;
    }

    /** Trail que sigue a un sprite (proyectil, etc.) */
    trail(followTarget: any, key: string, frame?: string, cfg: any = {}) {
      const emitter = this.add.particles(followTarget.x, followTarget.y, key, frame as any, {
        lifespan: 250,
        speed: 0,
        frequency: 40,
        quantity: 1,
      ...
      const stop = () => { try { emitter.stop(); } catch {} };
      followTarget.on?.('destroy', stop);
      return emitter;
    }
    ...
    async create() {
      ...
      this.input.on('pointerdown', (p: any) => {
        // no colocar si haces click encima de una torre existente
        const overTower = this.towers.some(t =>
          PhaserLib.Math.Distance.Between(p.worldX, p.worldY, t.sprite.x, t.sprite.y) < this.map.tileSize * 0.5
        );
        if (overTower) return;
      ...
      this.input.keyboard?.on('keydown', (e: KeyboardEvent) => {
        ...
        if (e.key === 'Enter') this.nextWaveNow(); // opcional: lanzar oleada ya
      });
    ...
    nextWaveNow() {
      // Si quieres botón/tecla para forzar siguiente oleada, puedes llamar a esta función
      // Aquí podrías gestionar una cola/cancelación de delayedCalls si fuese necesario
    }
    ...
    fireAt(t: { sprite: any; model: TowerModel }, target: any) {
      ...
      // Trail por familia
      if (m.fam === 'electric') this.trail(p, 'projectiles', 'Lightning Bolt', { frequency: 30 });
      if (m.fam === 'fire')     this.trail(p, 'projectiles', 'Fireball',       { frequency: 35 });
      if (m.fam === 'frost')    this.trail(p, 'projectiles', 'Ice Shard',      { frequency: 35 });
    ...
    doHit(proj: any, enemy: any) {
      ...
      // VFX de impacto por familia
      if (fam === 'electric') this.burst(e.x, e.y, 'fx', 'Electric Discharge', { lifespan: 200, speed: { min: 120, max: 220 } }, 18);
      if (fam === 'fire')     this.burst(e.x, e.y, 'fx', 'Fire Explosion',     { lifespan: 240 }, 16);
      if (fam === 'frost')    this.burst(e.x, e.y, 'fx', 'Ice Explosion',      { lifespan: 240 }, 16);
      ...
      proj.destroy();
      try { this.sound.play('hit', { volume: 0.25 }); } catch {}
    }
