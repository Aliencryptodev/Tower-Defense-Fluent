...
// Mapeo exacto a tus nombres con espacios:
...
// Dibuja una celda de camino combinando horizontal/vertical del MISMO tile con rotación 0/90
function drawPathCell(scene: Phaser.Scene, gx: number, gy: number, biome: keyof typeof TERRAIN_FRAMES, mask: number) {
  ...
  const horiz = (mask & 2) || (mask & 8); // E o W
  const vert  = (mask & 1) || (mask & 4); // N o S
  ...
  useEffect(() => {
    ...
    class TD extends Phaser.Scene {
      ...
      preload() {
        ...
        // Enemigos por tamaño (usa los que tengas)
      ...
      create() {
        ...
        // camino simple (dos líneas horizontales) usando "Grass Path"
        const grid = [
          Array(GRID_W).fill(1),
          Array(GRID_W).fill(0),
          Array(GRID_W).fill(1),
        ...
        // Animaciones de torre y enemigo (fallback si no hay secuencias)
        ...
        // Colocar torre con click (usa tu frost_idle_1.png si está)
        ...
        // Oleada dummy de goblins andando
        this.time.delayedCall(500, () => {
          const pathY = TILE/2; // fila superior del camino
          const startX = GRID_W*TILE + 40;
          for (let i=0; i<8; i++) {
            const e = this.add.sprite(startX + i*50, pathY, 'enemies32');
            e.play('goblin_walk'); e.setDepth(e.y);
            this.tweens.add({
              targets: e, x: -40, duration: 8000 + i*200, onComplete: () => {
                e.play('goblin_death'); this.time.delayedCall(400, ()=> e.destroy());
              }
            });
          }
        });

        // Simular disparo de torres (si no tienes attack, hace play al idle otra vez)
        this.time.addEvent({
